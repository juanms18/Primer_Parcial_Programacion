...
int ingresarComoUsuarioOAdmin(Usuario * eUsuario , int len , Producto * listProducto , int lenProducto , Tracking * listaTracking , int lenTracking)
{
	...
	if(eUsuario != NULL && len > 0 )
	{
		...
		if(llamaFuncion==1)
		{
			...
			if(tipo==ADMIN)
			{
              inicioMenuAdmin(eUsuario, len, listProducto, lenProducto, listaTracking, lenTracking);
			}
			retorno=1;
		}
...
int inicioMenuAdmin (Usuario * listaUsuario, int lenUsuario , Producto * listaProductos , int lenProducto, Tracking * listaTracking , int lenTracking )
{
	int opcionAdmin;
	...
	while(opcionAdmin!=0)
	{
		...
		if(opcionAdmin==5)
		{
          verTrackingGlobal(listaTracking, lenTracking);
		}
	}
}
